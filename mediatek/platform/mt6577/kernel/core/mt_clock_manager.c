...
static int disable_subsys_locked(enum mt65xx_subsys_id id, char *mod_name)
{
    ...
    if (id == MT65XX_SUBSYS_MM1) {
        ...
        if (unlikely(get_chip_eco_ver() == CHIP_E1)) {
            ...
            disable_irq(MT_SMI_LARB0_MMU_IRQ_ID);    
            disable_irq(MT_SMI_LARB1_MMU_IRQ_ID);    
            disable_irq(MT_SMI_LARB2_MMU_IRQ_ID);    

	/* FIXME: For Linux Early Porting */
#if 0
            mt_irq_set_polarity(MT_SMI_LARB0_MMU_IRQ_ID, MT65xx_POLARITY_LOW);
            mt_irq_set_polarity(MT_SMI_LARB1_MMU_IRQ_ID, MT65xx_POLARITY_LOW);
            mt_irq_set_polarity(MT_SMI_LARB2_MMU_IRQ_ID, MT65xx_POLARITY_LOW);
#endif 
        }
#if 0
