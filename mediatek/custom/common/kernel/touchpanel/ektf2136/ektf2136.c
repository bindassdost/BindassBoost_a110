...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
    ...
    ret = i2c_master_recv(i2c_client, raw_data_buf, 18);  // i2c support > 8bits transfer
...
// for 5 fingers	
    if ((raw_data_buf[0] == 0x5D))  //6D for packet <=8 , 5D for packet > 8
    {   
...
{
    TPD_DEBUG("[elan::] MediaTek elan touch panel driver init\n");
